export default {
    settings: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 OriginCoder 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 OriginCoder 用户登录',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，OriginCoder 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      upstreamBillingProbe: {
        title: '上游倍率自动探测',
        description: '定期获取 API Key 账号所连接上游 OriginCoder 站点声明的计费倍率；只有另行开启“同步上游声明倍率”的账号才会更新账号倍率。',
        enabled: '启用全局自动探测',
      ...
      payment: {
        ...
        field_nowpaymentsIpnSecretHint: 'NOWPayments 后台 Settings → Payments → IPN 里生成的密钥，用于校验回调签名。必须同时在 NOWPayments 后台把 IPN callback URL 填成本站的 /api/v1/payment/webhook/nowpayments，否则订单永远停在待支付。',
        field_nowpaymentsPayCurrencyHint: '用户实际转账的币种与网络。网络选错会导致用户资金无法找回，请与 NOWPayments 后台已启用的币种保持一致。',
        field_nowpaymentsApiBaseHint: '留空使用官方地址 https://api.nowpayments.io/v1，仅在自建代理时才需要修改。',
        field_paymentCurrencyHint: '默认 CNY。Stripe、Airwallex 与 NOWPayments 可按账户支持从下拉项选择 HKD、USD 等币种；微信、支付宝、易支付仍按 CNY。NOWPayments 按此币种计价，再按实时汇率折算成链上应付数量。',
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 OriginCoder 订单中，上游 type 会作为易支付 type 参数提交。',
        ...
        nowpaymentsWebhookHint: "把这个地址填到 NOWPayments 后台 Settings → Payments → IPN 的 callback URL 里，否则用户付款后订单不会自动完成。",
        nowpaymentsGuideSummary: "API Key 与 IPN Secret 在 NOWPayments 后台 Settings 里生成；IPN Secret 用于校验回调签名，缺失或填错会导致所有到账通知被拒绝。",
        nowpaymentsGuideNote: "收款币种决定用户实际转账的链。选定后不要在有未完成订单时更改——已下发的收款地址属于原来那条链，改了会导致对账失败。链上转账不可逆，本服务商不支持程序化退款。",
      ...
      purchaseWebhook: {
        title: '购买成功群机器人推送',
        description: '用户充值或购买订阅成功后，向企业微信 / 钉钉群机器人推送 Markdown 通知卡片。',
        enabled: '启用群机器人推送',
        enabledHint: '关闭后不会推送任何购买事件。',
        rechargeEvent: '余额充值成功',
        subscriptionEvent: '订阅购买成功',
        wecomUrl: '企业微信群机器人 Webhook',
        wecomUrlPlaceholder: '完整 Webhook 地址，或仅填 key',
        wecomUrlHint: '在企业微信群「群机器人」中添加机器人后复制 Webhook 地址；留空表示不推送到企业微信。',
        dingtalkUrl: '钉钉群机器人 Webhook',
        dingtalkUrlPlaceholder: '完整 Webhook 地址，或仅填 access_token',
        dingtalkUrlHint: '在钉钉群「智能群助手」中添加自定义机器人后复制 Webhook 地址；留空表示不推送到钉钉。',
        dingtalkSecret: '钉钉加签密钥',
        dingtalkSecretPlaceholder: '机器人安全设置选择「加签」时填写（SEC 开头）',
        dingtalkSecretHint: '机器人安全设置使用「关键词」或「IP 白名单」时无需填写。',
        dingtalkSecretConfiguredPlaceholder: '已配置，留空保持不变',
        dingtalkSecretConfiguredHint: '已保存加签密钥，留空则保持不变；清空钉钉 Webhook 会一并清除密钥。',
        test: '测试推送',
        testing: '推送中...',
        testSuccess: '测试消息已发送',
        testFailed: '测试推送失败',
