...
describe('resolveDocumentTitle', () => {
  ...
  it('站点名为空时，回退默认站点名', () => {
    expect(resolveDocumentTitle('Dashboard', '')).toBe('Dashboard - OriginCoder')
    expect(resolveDocumentTitle(undefined, '   ')).toBe('OriginCoder')
  })

  it('后端仍返回旧默认名时，仅在页面标题中展示新品牌名', () => {
    expect(resolveDocumentTitle('Dashboard', 'Sub2API')).toBe('Dashboard - OriginCoder')
  })
