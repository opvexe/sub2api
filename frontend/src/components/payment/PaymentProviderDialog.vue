  ...
  >
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-dark-700">
        ...
        <!-- 服务商 Webhook 提示 -->
        <div v-if="providerWebhookUrl" class="mt-3 rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/50">
          <p class="text-xs text-gray-600 dark:text-dark-300">
            {{ t(providerWebhookHint) }}
          </p>
          <code class="mt-1 block break-all rounded bg-gray-200 px-2 py-1 text-xs text-gray-800 dark:bg-dark-700 dark:text-gray-200">
            {{ providerWebhookUrl }}
          </code>
          <p v-if="form.provider_key === 'stripe'" class="mt-2 text-xs leading-relaxed text-gray-600 dark:text-dark-300">
