...
const healthScoreColor = computed(() => {
  if (isSystemIdle.value) return '#a8a196' // gray-400
  const score = healthScoreValue.value
  if (score == null) return '#a8a196'
            ...
            >
              <div class="rounded-xl bg-white p-4 shadow-xl ring-1 ring-black/5 dark:bg-dark-800 dark:ring-white/10">
                <h4 class="mb-3 border-b border-gray-100 pb-2 text-sm font-bold text-gray-900 dark:border-dark-700 dark:text-white flex items-center gap-2">
                  <Icon name="brain" size="sm" class="text-gray-400 dark:text-gray-500" />
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    <div class="mt-0.5 shrink-0">
                      ...
                      <svg v-else class="h-4 w-4 text-gray-400 dark:text-gray-500" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 1;">
          <div class="flex items-center justify-between">
            ...
            <button
              v-if="!props.fullscreen"
              class="text-[10px] font-bold text-primary-600 hover:underline dark:text-primary-400"
              type="button"
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 3;">
          <div class="flex items-center justify-between">
            ...
            <button v-if="!props.fullscreen" class="text-[10px] font-bold text-primary-600 hover:underline dark:text-primary-400" type="button" @click="openErrorDetails('request')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
        ...
        <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-900" style="order: 6;">
          <div class="flex items-center justify-between">
            ...
            <button v-if="!props.fullscreen" class="text-[10px] font-bold text-primary-600 hover:underline dark:text-primary-400" type="button" @click="openErrorDetails('upstream')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
      ...
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div class="rounded-xl bg-gray-50 p-3 dark:bg-dark-900">
          <div class="flex items-center justify-between gap-2">
            ...
            <button v-if="!props.fullscreen" class="text-[10px] font-bold text-primary-600 hover:underline dark:text-primary-400" type="button" @click="openJobsDetails">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
